(function () {
  ...
  function allDays(grid) {
    let days = [];
    grid.querySelectorAll('[data-day]').forEach(function (r) { days.push(r.dataset.day); });
    return days;
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (Object.hasOwn(btn.dataset, 'addFrame')) {
      e.preventDefault();
      frameBox(dayRow(grid, btn.dataset.addFrame))
        .appendChild(makeFrame(grid, btn.dataset.addFrame, '', ''));
