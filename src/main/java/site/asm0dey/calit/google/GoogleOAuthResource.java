...
public class GoogleOAuthResource {
    ...
    public Response connect() {
        // @Authenticated guarantees a principal, so this won't NPE-on-unbox
    ...
    public Response callback(
        ...
        if (error != null) {
            ...
            LOG.warnf("Google OAuth callback returned error: %s", error.replace('\r', ' ').replace('\n', ' '));
            return Response
        ...
        if (ownerId == null) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Invalid or expired OAuth state").build();
        }
        ...
        return Response.status(Response.Status.FOUND).location(URI.create("/me/google")).build();
    }
}
