...
class BookingPostTest {
    ...
    void seedApprovalType() {
        ...
        MeetingType t = new MeetingType();
        t.ownerId = ownerId;
        t.name = "Approval Confirm Type";
        t.slug = "approval-confirm";
        t.durationMinutes = 60;
        t.locationType = LocationType.GOOGLE_MEET;
    ...
    private String firstSlot(String slug) {
        String html = given().when().get("/bob/" + slug).then().statusCode(200).extract().asString();
        var startUtc =
    ...
    void postCreatesBookingAndShowsMeetLinkAndManageToken() {
        ...
        when(calendarPort.createEvent(anyLong(), any(), any(), any(), any(), any(), any(), anyBoolean(), any()))
            .thenReturn(
                    new CreatedEvent(
                            "evt-1",
                            "https://meet.google.com/known-test-link",
                            "https://calendar.google.com/evt-1",
                            null
                    )
            );
        seed();
        // the absolute UTC instant the invitee submits (…Z)
        ...
        given()
            ...
            // required custom field populated
            .formParam(
                    // required custom field populated
            "answers.company",
                    "Acme Corp")
    ...
    void postForApprovalTypeShowsPendingWordingNotConfirmation() {
        ...
        var chosen = firstSlot("approval-confirm");
        given()
            .contentType("application/x-www-form-urlencoded")
            .formParam("startUtc", chosen)
    ...
    void postWithMissingRequiredAnswerReRendersFormWith422NotServerError() {
        ...
        given()
            ...
            .then()
            .statusCode(200)
            // inline validation message rendered
            .body(containsString("alert-error"))
            // back on the booking form, not a 500
