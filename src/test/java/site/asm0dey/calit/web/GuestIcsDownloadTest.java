...
class GuestIcsDownloadTest {
    ...
    void confirmationPageOffersTheIcsDownloadWhenGoogleIsNotConnected() {
        ...
        GuestBookingFixture
            .book(this.getClass().getSimpleName() + "-link")
            .then()
            .statusCode(200)
    ...
    void confirmationPageHidesTheIcsDownloadWhenGoogleIsConnected() {
        ...
        GuestBookingFixture
            .book(this.getClass().getSimpleName() + "-hidden")
            .then()
            .statusCode(200)
    ...
    void icsEndpointServesTheCalendarEntry() {
        ...
        String token =
                GuestBookingFixture.manageTokenOf(GuestBookingFixture.book(this.getClass().getSimpleName() + "-dl"));
    ...
    void icsEndpointStaysOpenWhenGoogleIsConnected() {
        when(calendarPort.isConnected(anyLong())).thenReturn(false);
        String token =
                GuestBookingFixture.manageTokenOf(GuestBookingFixture.book(this.getClass().getSimpleName() + "-open"));
        ...
        when(calendarPort.isConnected(anyLong())).thenReturn(true);
        given().when().get("/booking/" + token + "/invite.ics").then().statusCode(200).body(
                containsString("UID:" + token)
        );
    }
