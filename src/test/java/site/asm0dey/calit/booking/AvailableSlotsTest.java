...
class AvailableSlotsTest {
    ...
    void busyInsideBufferZoneRemovesAdjacentSlot() {
        ...
        // exactly one raw slot 09:00-10:00
        globalRule(DAY.getDayOfWeek(), "09:00", "10:00");
        when(calendarPort.isConnected(anyLong())).thenReturn(true);
    ...
    void minNoticeDropsNearTermSlots() {
        ...
        // ~50 years -> well past any near-term slot
        t.minNoticeMinutes = 60 * 24 * 365 * 50;
        t.persist();
    ...
    void horizonDropsFarFutureSlots() {
        ...
        // only ~tomorrow is bookable; a 30-day-out slot is past the horizon
        t.horizonDays = 1;
        t.persist();
    ...
    private MeetingType meetingType(String slug, int minutes, int bufBefore, int bufAfter) {
        ...
        // ~136 years: keeps the DAY slot comfortably inside the horizon
