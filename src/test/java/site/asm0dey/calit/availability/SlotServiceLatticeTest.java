...
class SlotServiceLatticeTest {
    ...
    private Set<Instant> starts(MeetingType t, Long hostOwnerId, ZoneId latticeZone, LocalDate from, LocalDate to) {
        return slotService
            .generateRawSlots(t, hostOwnerId, from, to, latticeZone, t.durationMinutes)
            .stream()
    ...
    void aTwentyNineMinuteCadenceStillIntersectsAcrossAFourHourFortyFiveOffset() {
        ...
        // no extra lengths -> step falls back to 29
        MeetingType t = seedType("lattice-29", 29);
    ...
    void anAllKathmanduTeamKeepsRoundLocalTimes() {
        ...
        List<LocalTime> local = starts(t, kathmandu1, latticeZone)
            .stream()
    ...
    void aNullLatticeZoneKeepsWindowAnchoringForSingleHost() {
        ...
        List<LocalTime> local = slotService
            .generateRawSlots(t, CREATOR, MONDAY, MONDAY, null, 45)
            .stream()
    ...
    void aFallBackHourYieldsBothInstantsOnTheLatticePath() {
        ...
        List<Instant> host1Starts = slotService
            .generateRawSlots(t, berlin1, fallBackDay, fallBackDay, latticeZone, 60)
            .stream()
            .map(s -> s.start().toInstant())
            .sorted()
            .toList();
        ...
        List<Instant> host2Starts = slotService
            .generateRawSlots(t, berlin2, fallBackDay, fallBackDay, latticeZone, 60)
            .stream()
            .map(s -> s.start().toInstant())
            .sorted()
            .toList();
